    ...
    try {
      ...
      if (mode === 'livraison') {
        ...
        if (!isInDeliveryZone(lat, lng)) {
          setAddressError('Votre adresse est hors zone de livraison SEMOUS (max 7 km). Vous pouvez choisir le retrait.')
        ...
        const { data: addr } = await supabase.from('addresses').insert({
          ...
          latitude: feature.geometry.coordinates[1], longitude: lng,
        }).select().single()
        addressId = addr?.id
      }
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div><label className="text-xs font-medium mb-1 block">Prénom *</label><input required className="input-field" value={form.prenom} onChange={e => f('prenom', e.target.value)} placeholder="Votre prénom" /></div>
              <div><label className="text-xs font-medium mb-1 block">Email *</label><input required type="email" className="input-field" value={form.email} onChange={e => f('email', e.target.value)} placeholder="votre@email.com" /></div>
              {mode === 'livraison' && (
                <div className="sm:col-span-2"><label className="text-xs font-medium mb-1 block">Téléphone *</label><input required className="input-field" value={form.telephone} onChange={e => f('telephone', e.target.value)} placeholder="06 XX XX XX XX" /></div>
              )}
            </div>
          </div>
          ...
          {mode === 'livraison' && (
            <div className="card p-5">
              ...
              <div className="flex flex-col gap-3">
                <div><label className="text-xs font-medium mb-1 block">Adresse *</label><input required className="input-field" value={form.rue} onChange={e => { f('rue', e.target.value); setAddressError('') }} placeholder="Numéro et rue" /></div>
                <div><label className="text-xs font-medium mb-1 block">Complément</label><input className="input-field" value={form.complement} onChange={e => f('complement', e.target.value)} placeholder="Bâtiment, appartement..." /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs font-medium mb-1 block">Code postal *</label><input required className="input-field" value={form.code_postal} onChange={e => f('code_postal', e.target.value)} placeholder="31000" /></div>
                  <div><label className="text-xs font-medium mb-1 block">Ville *</label><input required className="input-field" value={form.ville} onChange={e => f('ville', e.target.value)} placeholder="Toulouse" /></div>
                </div>
        ...
        <div className="lg:col-span-1">
          <div className="card p-5 sticky top-20">
            ...
            <div className="border-t border-semous-gray-mid pt-3 flex flex-col gap-2 text-sm">
              ...
              {reduction > 0 && <div className="flex justify-between text-green-600"><span>Réduction</span><span>−{formatPrice(reduction)}</span></div>}
            ...
            <p className="text-xs text-semous-gray-text text-center mt-3">En attente de validation par SEMOUS.</p>
          </div>
        </div>
      </form>
    </div>
