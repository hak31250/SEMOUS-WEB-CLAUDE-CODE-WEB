...
export default function Account() {
  ...
  useEffect(() => {
    if (user) {
      ...
      supabase.from('orders').select('*, order_items(quantite, prix_unitaire_ttc, products(nom))')
  ...
  async function handleLogin(e) {
    e.preventDefault()
    setSubmitting(true)
    try { await signIn(loginForm); toast.success('Connexion réussie') }
    catch (err) { toast.error(err.message || 'Identifiants incorrects') }
  ...
  async function handleSignup(e) {
    ...
    } catch (err) { toast.error(err.message || 'Erreur à la création du compte') }
    finally { setSubmitting(false) }
  }
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={handleSignOut} className="btn-secondary flex items-center gap-2 text-sm"><LogOut size={14} />Déconnexion</button>
        </div>
          ...
          {editProfile ? (
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-medium mb-1 block">Prénom</label><input className="input-field" value={profileForm.prenom} onChange={e => setProfileForm(f => ({ ...f, prenom: e.target.value }))} /></div>
                <div><label className="text-xs font-medium mb-1 block">Nom</label><input className="input-field" value={profileForm.nom} onChange={e => setProfileForm(f => ({ ...f, nom: e.target.value }))} /></div>
              </div>
              <div><label className="text-xs font-medium mb-1 block">Téléphone</label><input className="input-field" value={profileForm.telephone} onChange={e => setProfileForm(f => ({ ...f, telephone: e.target.value }))} placeholder="06 XX XX XX XX" /></div>
              <div className="flex gap-3">
                ...
                <button onClick={saveProfile} disabled={profileSaving} className="btn-primary flex-1 flex items-center justify-center gap-2">
                  {profileSaving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}Sauvegarder
                </button>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => setEditProfile(true)} className="text-xs text-semous-gray-text hover:text-semous-black underline">Modifier</button>
            </div>
          )}
        </div>

        <h2 className="font-bold text-lg mb-4 flex items-center gap-2"><ShoppingBag size={18} />Mes commandes</h2>
        {ordersLoading ? (
            ...
            {orders.map(order => (
              <Link key={order.id} to={`/commande/confirmation/${order.id}`} className="card p-4 block hover:shadow-md transition-shadow">
                ...
                <div className="text-sm flex justify-between">
                  <span className="text-semous-gray-text">{order.order_items?.slice(0, 2).map(i => i.products?.nom).join(', ')}{(order.order_items?.length || 0) > 2 ? ` +${order.order_items.length - 2}` : ''}</span>
        ...
        <div className="mt-10">
          <h2 className="font-bold text-lg mb-4 flex items-center gap-2"><Shield size={18} />Vos droits RGPD</h2>
          <div className="card p-5 mb-4">
            <p className="text-sm text-semous-gray-text mb-4">
              ...
              Contactez-nous à <a href="mailto:contact@semous.fr" className="underline">contact@semous.fr</a>.
            </p>
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={handleDownloadData} disabled={downloadingData}
                ...
                {downloadingData ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}Télécharger mes données
              </button>
              {deleteSent ? (
                <div className="flex-1 text-center text-sm text-semous-gray-text bg-semous-gray rounded-xl px-4 py-2 flex items-center justify-center">Demande envoyée. Traitement sous 30 jours.</div>
              ) : (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <div className="card p-6 max-w-sm w-full">
              ...
              <p className="text-sm text-semous-gray-text mb-5">
                Demande de suppression de votre compte et de vos données personnelles. Traitée sous <strong>30 jours</strong>.
              </p>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteModal(false)} className="btn-secondary flex-1">Annuler</button>
                <button onClick={handleRequestDeletion} disabled={deletingAccount}
                  className="flex-1 bg-red-600 text-white rounded-xl py-2.5 font-semibold text-sm flex items-center justify-center gap-2 hover:bg-red-700 disabled:opacity-60">
                  {deletingAccount ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}Confirmer
                </button>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-10">
      ...
      <p className="text-sm text-semous-gray-text text-center mb-8">Commandez plus vite, accédez à votre historique.</p>
      ...
      {tab === 'login' ? (
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          <div><label className="text-xs font-medium mb-1 block">Email</label><input required type="email" className="input-field" value={loginForm.email} onChange={e => setLoginForm(f => ({ ...f, email: e.target.value }))} placeholder="votre@email.com" autoFocus /></div>
          <div>
            <label className="text-xs font-medium mb-1 block">Mot de passe</label>
            <div className="relative">
              <input required type={showPwd ? 'text' : 'password'} className="input-field pr-10" value={loginForm.password} onChange={e => setLoginForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" />
              <button type="button" onClick={() => setShowPwd(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-semous-gray-text">{showPwd ? <EyeOff size={16} /> : <Eye size={16} />}</button>
            </div>
          </div>
          <button type="submit" disabled={submitting} className="btn-primary flex items-center justify-center gap-2">
            {submitting ? <Loader2 size={16} className="animate-spin" /> : <User size={16} />}Se connecter
          </button>
        </form>
      ) : (
        <form onSubmit={handleSignup} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-medium mb-1 block">Prénom *</label><input required className="input-field" value={signupForm.prenom} onChange={e => setSignupForm(f => ({ ...f, prenom: e.target.value }))} placeholder="Prénom" autoFocus /></div>
            <div><label className="text-xs font-medium mb-1 block">Nom</label><input className="input-field" value={signupForm.nom} onChange={e => setSignupForm(f => ({ ...f, nom: e.target.value }))} placeholder="Nom" /></div>
          </div>
          <div><label className="text-xs font-medium mb-1 block">Email *</label><input required type="email" className="input-field" value={signupForm.email} onChange={e => setSignupForm(f => ({ ...f, email: e.target.value }))} placeholder="votre@email.com" /></div>
          <div><label className="text-xs font-medium mb-1 block">Téléphone</label><input className="input-field" value={signupForm.telephone} onChange={e => setSignupForm(f => ({ ...f, telephone: e.target.value }))} placeholder="06 XX XX XX XX" /></div>
          <div>
            <label className="text-xs font-medium mb-1 block">Mot de passe *</label>
            <div className="relative">
              <input required type={showPwd ? 'text' : 'password'} minLength={8} className="input-field pr-10" value={signupForm.password} onChange={e => setSignupForm(f => ({ ...f, password: e.target.value }))} placeholder="8 caractères minimum" />
              <button type="button" onClick={() => setShowPwd(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-semous-gray-text">{showPwd ? <EyeOff size={16} /> : <Eye size={16} />}</button>
            </div>
          </div>
          <button type="submit" disabled={submitting} className="btn-primary flex items-center justify-center gap-2">
            {submitting ? <Loader2 size={16} className="animate-spin" /> : null}Créer mon compte
          </button>
        </form>
      )}
      <p className="text-xs text-semous-gray-text text-center mt-6">
        Vous pouvez aussi commander <Link to="/commande" className="underline">sans créer de compte</Link>.
      </p>
    </div>
  )
}
