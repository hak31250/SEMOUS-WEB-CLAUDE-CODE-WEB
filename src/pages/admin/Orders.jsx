        ...
        <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-1 max-w-sm">
            <div className="relative flex-1">
              ...
              <input className="input-field pl-9 text-sm" placeholder="N° commande..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          ...
          <div className="flex flex-col gap-2">
            {filtered.map(order => (
              <button key={order.id} onClick={() => openDetail(order)}
                className={`card p-4 text-left hover:shadow-md transition-shadow w-full ${selectedOrder?.id === order.id ? 'ring-2 ring-semous-black' : ''}`}>
                ...
                {order.note_interne && (
                  <p className="text-xs text-yellow-700 mt-2 bg-yellow-50 rounded px-2 py-0.5 text-left">📝 {order.note_interne}</p>
                )}
              </button>
            ))}
            {filtered.length === 0 && <p className="text-center py-12 text-semous-gray-text">Aucune commande trouvée</p>}
          </div>
        )}
      </div>
    ...
    <div className="card p-5 sticky top-20 overflow-y-auto max-h-[calc(100vh-6rem)]">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={onPrint} title="Imprimer ticket" className="p-1.5 text-semous-gray-text hover:text-semous-black"><Printer size={16} /></button>
          <button onClick={() => setNoteOpen(n => !n)} title="Note interne" className={`p-1.5 rounded ${noteOpen ? 'bg-yellow-100 text-yellow-700' : 'text-semous-gray-text hover:text-semous-black'}`}><MessageSquare size={16} /></button>
          <button onClick={onClose} className="p-1.5 text-semous-gray-text hover:text-semous-black"><X size={16} /></button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 text-sm mb-4">
        ...
        <div className="flex justify-between"><span className="text-semous-gray-text">Mode</span><span className="capitalize font-medium">{order.type}</span></div>
        {order.mode_paiement && <div className="flex justify-between"><span className="text-semous-gray-text">Paiement</span><span className="capitalize">{order.mode_paiement}</span></div>}
        <div className="flex justify-between"><span className="text-semous-gray-text">Total TTC</span><span className="font-bold">{formatPrice(order.total_ttc)}</span></div>
        {order.frais_livraison > 0 && <div className="flex justify-between"><span className="text-semous-gray-text">Livraison</span><span>{formatPrice(order.frais_livraison)}</span></div>}
        {order.reduction > 0 && <div className="flex justify-between"><span className="text-semous-gray-text">Réduction</span><span className="text-green-600">-{formatPrice(order.reduction)}</span></div>}
        <div className="flex justify-between"><span className="text-semous-gray-text">Date</span><span>{formatDate(order.created_at)}</span></div>
        {addressStr && <div className="flex justify-between gap-2"><span className="text-semous-gray-text shrink-0">Adresse</span><span className="text-right text-xs">{addressStr}</span></div>}
      </div>

      {noteOpen && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
          <p className="text-xs font-semibold text-yellow-800 mb-2">Note interne</p>
          <textarea className="w-full text-xs bg-white border border-yellow-200 rounded-lg p-2 resize-none focus:outline-none focus:ring-1 focus:ring-yellow-400" rows={3}
            value={note} onChange={e => setNote(e.target.value)} placeholder="Note visible équipe uniquement..." />
          <button onClick={() => { onSaveNote(order.id, note); setNoteOpen(false) }}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {statusActions[order.statut]?.map(action => (
          <button key={action.status} onClick={() => onUpdateStatus(order.id, action.status)} className={`${action.style} text-sm py-2 px-4 flex-1`}>{action.label}</button>
        ))}
      </div>

      {order.history?.length > 0 && (
        <div className="border-t border-semous-gray-mid pt-4">
          <button onClick={() => setHistoryOpen(h => !h)} className="flex items-center gap-2 text-sm font-semibold w-full text-left">
