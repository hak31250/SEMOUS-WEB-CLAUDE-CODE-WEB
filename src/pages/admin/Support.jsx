...
export default function Support() {
  ...
  async function sendReply() {
    ...
    const { error } = await supabase.from('support_messages').insert({
      ticket_id: selected.id, auteur: 'admin', contenu: reply.trim(),
    })
              ...
              <button key={ticket.id} onClick={() => openTicket(ticket)}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-semous-gray-text mt-0.5">{ticket.email_client} · {formatDate(ticket.created_at)}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className={`badge text-xs ${STATUS_COLORS[ticket.statut] || 'bg-gray-100 text-gray-500'}`}>{ticket.statut}</span>
                    <span className={`badge text-xs ${PRIORITY_COLORS[ticket.priorite] || 'bg-gray-100'}`}>{ticket.priorite}</span>
                  </div>
                </div>
              </button>
            ))}
          ...
          <div className="card flex-1 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-semous-gray-mid flex items-start justify-between gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button onClick={() => setSelected(null)} className="text-semous-gray-text hover:text-semous-black"><X size={16} /></button>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 min-h-0">
              {messages.length === 0 && <p className="text-center text-semous-gray-text text-sm py-8">Aucun message</p>}
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.auteur === 'admin' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm
                    ${msg.auteur === 'admin' ? 'bg-semous-black text-white rounded-br-sm' : 'bg-semous-gray text-semous-black rounded-bl-sm'}`}>
                    <p className="leading-relaxed">{msg.contenu}</p>
                    <p className={`text-xs mt-1 ${msg.auteur === 'admin' ? 'text-white/50' : 'text-semous-gray-text'}`}>{formatDate(msg.created_at)}</p>
                  </div>
                </div>
              ))}
            ...
            {selected.statut !== 'resolu' && selected.statut !== 'ferme' && (
              <div className="p-4 border-t border-semous-gray-mid">
                <div className="flex gap-2">
                  <textarea className="input-field flex-1 resize-none text-sm" rows={2} value={reply}
                    onChange={e => setReply(e.target.value)} placeholder="Répondre au client..."
                    onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendReply() } }} />
                  <button onClick={sendReply} disabled={!reply.trim() || sending} className="btn-primary px-3 self-end flex items-center">
