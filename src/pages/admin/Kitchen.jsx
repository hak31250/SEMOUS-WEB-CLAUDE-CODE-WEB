...
export default function Kitchen() {
  ...
  async function updateStatus(order, newStatus) {
    ...
    if (newStatus === 'en_preparation') {
      // auto-print ticket when entering preparation
      ...
      {ruptureModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-semous-gray-text mb-2">
              Produit en rupture : <strong>{ruptureModal.productNom}</strong>
            </p>
            <p className="text-sm text-semous-gray-text mb-5">
              Commande : <strong>{ruptureModal.order.numero}</strong>
            </p>
            <div className="flex flex-col gap-2">
              <button onClick={() => handleRupture('noter')} className="btn-primary text-sm py-3">
                Marquer rupture + continuer la commande
              </button>
              <button onClick={() => handleRupture('refuser')} className="border border-red-400 text-red-600 text-sm py-3 rounded-xl hover:bg-red-50">
                Refuser la commande
              </button>
              <button onClick={() => setRuptureModal(null)} className="btn-secondary text-sm py-2">
                Annuler
              </button>
            </div>
          </div>
  ...
  return (
    ...
    `}>
      ...
      <div className="flex flex-col gap-2">
        {order.order_items?.map((item, i) => (
          <div key={i} className="flex items-start gap-2 text-sm">
            ...
            {item.products?.id && (
              <button
                onClick={() => onSignalRupture(item.products.id, item.products.nom)}
                className="text-xs px-2 py-1 rounded border border-orange-300 text-orange-600 hover:bg-orange-50 shrink-0"
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-current/10">
        ...
        {[10, 20, 30].map(min => (
          <button key={min} onClick={() => onAddDelay(min)} className="text-xs px-2 py-2 rounded-lg border border-current/20 hover:bg-black/5">
            +{min}m
          </button>
        ))}
        <button
          ...
          className="text-xs px-2 py-2 rounded-lg border border-current/20 hover:bg-black/5"
