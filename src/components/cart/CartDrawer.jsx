...
import { useCartStore, LIVRAISON_MINIMUM } from '@/store/cartStore'
import { formatPrice } from '@/utils/format'
import CartItem from './CartItem'

export default function CartDrawer({ open, onClose }) {
  const navigate = useNavigate()
  const { items, mode, sousTotal, fraisLivraison, reduction, totalFinal, isDeliveryEligible } = useCartStore()
          ...
          <div className="border-t border-semous-gray-mid px-5 py-4 flex flex-col gap-3">
            ...
            {mode === 'livraison' && !isDeliveryEligible && (
              <p className="text-xs text-orange-600 bg-orange-50 rounded-lg px-3 py-2">
                Minimum de commande pour la livraison : {formatPrice(LIVRAISON_MINIMUM)}.
